...
export const getAppointments = () => async dispatch => {
    try {
        const res = await axiosInstance.get('/api/authUser');
        dispatch({
...
export const addAppointment = (doctorId, formData, history) => async dispatch => {
    ...
    try {
        const res = await axiosInstance.post(`/api/appointment/${doctorId}`,formData, config);
        dispatch({
...
export const deleteAppointment = (appointmentId) => async dispatch => {
    try {
        const res = await axiosInstance.delete(`/api/authUser/${appointmentId}`);
        dispatch({
...
export const deleteAccountUser = () => async dispatch => {
    if(window.confirm('Are you sure? this can not be undone!')) {
        try {
            await axiosInstance.delete('/api/authUser');
